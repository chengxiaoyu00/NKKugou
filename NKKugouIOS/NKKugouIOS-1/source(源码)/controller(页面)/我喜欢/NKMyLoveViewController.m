...
#import "NKMyLoveViewController.h"
...
@interface NKMyLoveViewController ()
{
    NKEditBar* _editBar;
    BOOL _isEditBarShow;
...
@property (weak, nonatomic) IBOutlet UITableView *myLoveTableView;

@end

@implementation NKMyLoveViewController
...
- (void)viewDidLoad {
    ...
    //1.拿出来原来的frame
    CGRect frame = _myLoveTableView.frame;
    //2.修改部分属性
    frame.size.height = [UIScreen mainScreen].applicationFrame.size.height*(568.0-22.0)/568.0;
    //3.设置回来
    _myLoveTableView.frame = frame;
    
    //设置编辑栏
    [self setupEditBar];
    
    //tableView的Cell之间的分割线去掉
    _myLoveTableView.separatorColor = [UIColor clearColor];
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    //将我喜欢列表拷贝到当前播放音乐列表中。
    [NKPlayListTool sharedNKPlayListTool].curPlayMusicList = [NKPlayListTool sharedNKPlayListTool].myLoveMuiscList;
}
...
-(void)deleteList{
    ...
    //更新我喜欢的音乐列表
    [[NKPlayListTool sharedNKPlayListTool]deleteMuiscFromMyLoveList];
    //刷新tableView
    [_myLoveTableView reloadData];
}
...
-(void)playMusic:(NSNotification*)notify{
    ...
    [_myLoveTableView selectRowAtIndexPath:indexPath animated:YES scrollPosition:UITableViewScrollPositionMiddle];
...
-(void)geciVCPush{
    ...
    [self performSegueWithIdentifier:@"myLoveGeciPush" sender:self];
}
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    NSLog(@"count %li", [NKPlayListTool sharedNKPlayListTool].curPlayMusicList.count);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    //View
    NKMyLoveTableViewCell* cell = [NKMyLoveTableViewCell myLoveTableViewCellWithTableView:tableView];
