...
@implementation NKSavePlayTableViewCell

+(instancetype)savePlayTableViewCellWithTableView:(UITableView*)tableView{
    
    static NSString* ID = @"myLoveCell";
    
    NKSavePlayTableViewCell* cell = [tableView dequeueReusableCellWithIdentifier:ID];
    
    if (cell == nil) {
        cell = [[NKSavePlayTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
    }
    return cell;
}
...
-(void)setupEditBar{
    ...
    NSArray* editItems = @[btn1, btn3, btn4];
...
-(void)deleteMuisc{
    ...
    if ([_delegate respondsToSelector:@selector(savePlayTableViewCell:didUpdateTableView:)]) {
        [_delegate savePlayTableViewCell:self didUpdateTableView:_indexPath];
    }
}
