...
@implementation NKLocMusicTableViewCell
+(instancetype)locMusicTableViewCellWithTableView:(UITableView*)tableView{
    
    static NSString* ID = @"localMusicCell";
    
    NKLocMusicTableViewCell* cell = [tableView dequeueReusableCellWithIdentifier:ID];
   
    if (cell == nil) {
        cell = [[NKLocMusicTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:ID];
    }
    return cell;
}
...
#pragma mark 将对应的歌曲添加到我喜欢列表
-(void)myLoveList{
    //把选中的歌曲保存到我喜欢列表中
    NKMusicModel* music = [NKPlayListTool sharedNKPlayListTool].curPlayMusicList[_indexPath.row];
    [[NKPlayListTool sharedNKPlayListTool]addOneMusicToMyLoveList:music];
}
...
-(void)saveList{
    ...
    if ([_delegate respondsToSelector:@selector(locMusicTableViewCell:didShowSaveListEditView:)]) {
        [_delegate locMusicTableViewCell:self didShowSaveListEditView:_indexPath];
    }
}

#pragma mark 将对应的歌曲从当前列表中删除
-(void)deleteMuisc{
    ...
    if ([_delegate respondsToSelector:@selector(locMusicTableViewCell:didUpdateTableView:)]) {
        [_delegate locMusicTableViewCell:self didUpdateTableView:_indexPath];
    }
}
